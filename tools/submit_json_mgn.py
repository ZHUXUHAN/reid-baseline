...
from modeling.mgn_baseline import MGNBaseline
...
def get_model(traindata_num_classes, modelname):
    ...
    model = MGNBaseline(traindata_num_classes, 2, pretrain_path, 'bnneck', 'after', 'resnet50_ibn_a', 'self', False)
...
def inference_samples(model, batch_size):  # 传入模型，数据预处理方法，batch_size
    ...
    for i in range(iter_n_query):
        ...
        with torch.no_grad():
            ...
            # batch_feature1 = model(batch_data)[1].detach().cuda()
            all_feature.append(batch_feature)
            # local_feature.append(batch_feature1)

            # flip_batch_data = flip_batch_data.to(device)
            flip_batch_feature = model(flip_batch_data).detach().cuda()
            # flip_batch_feature1 = model(flip_batch_data)[1].detach().cuda()
    ...
    np.save('distmatmgn.npy', distmat)
    print("SAVE DONE", distmat.shape)
...
if __name__ == "__main__":
    ...
    modelname = "/home/zxh/reid-baseline/model_rank/r50_ibn_mgn/resnet50_ibn_a_model_120.pth"
    batch_size = 128
